...
include { UMITOOLS_EXTRACT } from '../../modules/umitools/main.nf'
include { UMITOOLS_DEDUP } from '../../modules/umitools/main.nf'
include { CUTADAPT_QUALITY_TRIM } from '../../modules/cutadapt/main.nf'
include { CUTADAPT_TRIM_5PRIME_ADAPTER } from '../../modules/cutadapt/main.nf'
include { CUTADAPT_TRIM_3PRIME_ADAPTER } from '../../modules/cutadapt/main.nf'
...
workflow umitools_extract { 

    take : 
        data 
    
    main : 
        UMITOOLS_EXTRACT( data )
    
    emit : 
        fastq = UMITOOLS_EXTRACT.out.umi_extracted_fq
...
workflow cutadapt { 
    ...
    main : 
        CUTADAPT_QUALITY_TRIM( data )

        CUTADAPT_TRIM_5PRIME_ADAPTER( CUTADAPT_QUALITY_TRIM.out.fastqs )

        CUTADAPT_TRIM_3PRIME_ADAPTER( CUTADAPT_TRIM_5PRIME_ADAPTER.out.fastqs )
    
    emit : 
        fastqs = CUTADAPT_TRIM_3PRIME_ADAPTER.out.fastqs
...
workflow umitools_dedup { 
    ...
    emit : 
        bam = UMITOOLS_DEDUP.out.umi_dedup_bam
...
workflow {
    ...
    // parse umi from fastq
    umitools_extract( read_data_fq.out.reads )

    // trim 
    //trim( umitools_extract.out.fastq )
    ...
    // star align
    star_align( cutadapt.out.fastqs )

    // deduplicate with umi tools
    umitools_dedup( star_align.out.bams )
    ...
    */
    ...
    // bam to bigwig
    bam_to_bw( umitools_dedup.out.bam )
